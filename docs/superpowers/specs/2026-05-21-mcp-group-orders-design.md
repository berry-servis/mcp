...
- **Per-colleague quantity: min 1, no upper limit** (a colleague may order for their whole sub-team), distinct from the invoice tool's 20-100.
...
### `place_group_order`
- **Input:** `group_code: string`, `boxes: number` (>= 1, no max), `jam_addon: boolean`, `contact_email: string`, `contact_name: string`.
...
- `boxes` < 1 or not an integer, invalid email -> validation error listing the problems (mirror `request-strawberry-order.ts` style).
...
- `place-group-order.test.ts`: validation rejects (bad code, closed date, 0 boxes, bad email); on valid input (including a large box count), `createComgateCart` is called with the right metadata and the tool returns `pay_url` (mock `medusa.ts`).
...
- No auth on the public server (creating an unpaid cart is harmless; matches the existing public tools). Rohlik's MCP requires a logged-in account; we deliberately stay guest because a card payment is the only gate that matters.
- No order-tracking tool yet. Rohlik's MCP exposes order status/history; a `track_order(order_id)` tool is a clean future add once the channel proves out.
